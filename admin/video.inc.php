...
/*
	...
	$Id: video.inc.php 10319 2007-08-25 12:48:53Z heyond $
*/
...
<?
		...
		if(!$settings['vsiteid'] && $settingsnew['open']) {
			...
			if($settingsnew['bbname'] != $settings['bbname'] || $settingsnew['email'] != $settings['email'] || $settingsnew['url'] != $settings['url'] || $settingsnew['logo'] != $settings['logo'] || $settingsnew['type'] != $settings['type']) {
				$videoinfo = dfopen("http://union.bokecc.com/discuz2/edit.bo?siteid=$vsiteid&vpassword=$vpassword&siteName=".urlencode($settingsnew['bbnameu8'])."&siteUrl=".urlencode($settingsnew['url'])."&siteLogo=".urlencode($settingsnew['logo'])."&siteType=".intval($settingsnew['type'])."&siteEmail=".urlencode($settingsnew['email']));
				if($videoinfo == '-1') {
