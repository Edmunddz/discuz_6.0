...
/*
	...
	$Id: insenz.func.php 10284 2007-08-25 07:01:42Z liuqiang $
*/
...
function insenz_request($data, $return = TRUE, $fp = '') {
	...
	while($res = @fgets($fp, 1024)) {
		...
		if (strncmp($res, 'Content-Length: ', 16) == 0) {
			$res_length = intval(substr($res, 16));
		}
	}
	$res = @fread($fp, $res_length);
	...
	if(!$response = insenz_authcode($res, 'DECODE', $authkey)) {
		return array('status' => 1, 'data' => insenz_convert($res, 0));
	}
