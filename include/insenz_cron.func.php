...
/*
	...
	$Id: insenz_cron.func.php 10064 2007-08-23 05:35:59Z liuqiang $
*/
...
function insenz_runcron() {
	...
	$query = $db->query("SELECT c.id, c.type, c.fid AS virtualfid, c.tid, c.status, c.endtime, t.fid, t.authorid, t.author, t.subject, t.lastpost, t.displayorder FROM {$tablepre}campaigns c LEFT JOIN {$tablepre}threads t ON t.tid=c.tid WHERE c.nextrun<='$timestamp'");
	while($c = $db->fetch_array($query)) {
		...
		if($c['status'] == 1) {
			if($c['type'] != 4) {
				...
				if($moderated) {
					$db->query("INSERT INTO {$tablepre}threadsmod (tid, uid, username, dateline, action, expiration, status) VALUES ('$c[tid]', '".$_DCACHE[settings][insenz][uid]."', '".$_DCACHE[settings][insenz][username]."', '$timestamp', 'EST', '$c[endtime]', '0')", 'UNBUFFERED');
				}
