...
/*
	...
	$Id: cache.func.php 10339 2007-08-27 02:05:22Z liuqiang $
*/

define('DISCUZ_KERNEL_VERSION', '6.0.0');
define('DISCUZ_KERNEL_RELEASE', '20070828');
...
function getcachearray($cachename, $script = '') {
	...
	switch($cachename) {
		case 'settings':
			...
			if($data['insenz']['status']) {
				$softadstatus = intval($data['insenz']['softadstatus']);
				$hardadstatus = is_array($data['insenz']['hardadstatus']) ? implode(',', $data['insenz']['hardadstatus']) : '';
		...
		case 'smilies_display':
			while($type = $db->fetch_array($query)) {
				$squery = $db->query("SELECT id, code, url FROM {$tablepre}smilies WHERE type='smiley' AND code<>'' AND typeid='$type[typeid]'");
