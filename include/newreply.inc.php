...
/*
	...
	$Id: newreply.inc.php 10394 2007-08-28 02:24:23Z monkey $
*/
...
if(!supe_submitcheck() && !submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	if(isset($repquote) && (!$thread['price'] || $thread['special'])) {
		...
		if($editormode == 1 && $bbinsert && !(isopera() && isopera() < 9)) {
			$message = discuzcode($message, $post['smileyoff'], $post['bbcodeoff'], $post['htmlon'] & 1, $forum['allowsmilies'], $forum['allowbbcode'], $forum['allowimgcode'], $forum['allowhtml'], 0, 1);
		}
	}
